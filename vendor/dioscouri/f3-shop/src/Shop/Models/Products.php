...
class Products extends \Dsc\Mongo\Collections\Content
{
        ...
        if (!empty($filter_tags))
        {
            ...
            if (!empty($filter_tags)) 
            {
                ...
                else 
                {
					...
					//find the all variants
					foreach($filter_tags as $filter_key=>$filter_value){
						if(strpos($filter_value, ":")){
							$tags	=	explode(":", $filter_value);
							if(!empty($tags) && isset($tags['0'])){
								...
								switch($type){
									case 'color':
												...
												$color_filter[]	=	$filter_value;
									break;
									case 'size':
												if(!isset($size_filter)){
													$size_filter 	=	array();
												}
												$size_filter[]	=	$filter_value;
									break;
									case 'product':
												if(!isset($producttype_filter)){
													$producttype_filter 	=	array();
												}
												$producttype_filter[]	=	$filter_value;
									break;
								}
							}
					...
					$ids 	=	array();
					if(isset($color_filter) && !empty($color_filter)){
						$colorids = array( new \MongoId ); 
						 $agg = static::collection()->aggregate(array(
								array(
									'$match' => array('variants.tags' => array( '$in' => $color_filter ), 'categories.id'=>new \MongoId( (string) $filter_category_id ) )
								),
								array(
									'$unwind' => '$variants'
								),
								...
								array(
									'$match' => array('variants.tags' => array( '$in' => $color_filter ) )
								),                            
								array(
									'$group' => array(
										'_id' => '$_id'
					...
					if(isset($size_filter) && !empty($size_filter)){
						
						$sizeids = array( new \MongoId ); 
						 $agg = static::collection()->aggregate(array(
								array(
									'$match' => array('variants.tags' => array( '$in' => $size_filter ), 'categories.id'=>new \MongoId( (string) $filter_category_id ) )
								),
								array(
									'$unwind' => '$variants'
								),
								...
								array(
									'$match' => array('variants.tags' => array( '$in' => $size_filter ) )
								),                            
								array(
									'$group' => array(
										'_id' => '$_id'
							...
							if(empty($ids)){
								...
								//$ids	=		array_uintersect($ids, $sizeids, 'object_compare');
								$newIds 	=	array();
								foreach($ids as $key=>$value){
									$newobject_id 		=	$value->{'$id'};
									foreach($sizeids as $sizeKey=>$sizeValue){
										$newsizeobject_id 		=	$sizeValue->{'$id'};
										if($newobject_id==$newsizeobject_id){
											$newIds[] 	=	$value;
										}
									}
								}
					...
					if(isset($producttype_filter) && !empty($producttype_filter)){
						
						$sizeids = array( new \MongoId ); 
						 $agg = static::collection()->aggregate(array(
								array(
									'$match' => array('variants.tags' => array( '$in' => $producttype_filter ), 'categories.id'=>new \MongoId( (string) $filter_category_id ) )
								),
								array(
									'$unwind' => '$variants'
								),
								...
								array(
									'$match' => array('variants.tags' => array( '$in' => $producttype_filter ) )
								),                            
								array(
									'$group' => array(
										'_id' => '$_id'
							...
							if(empty($ids)){
								...
								foreach($ids as $key=>$value){
									$newobject_id 		=	$value->{'$id'};
									foreach($producttypeids as $sizeKey=>$sizeValue){
										$newsizeobject_id 		=	$sizeValue->{'$id'};
										if($newobject_id==$newsizeobject_id){
											$newIds[] 	=	$value;
										}
    ...
    protected function fetchItem()
    {
        ...
        $this->__cursor = $this->collection()->find($this->conditions());
    ...
    public static function buildVariants( $cast )
    {
        ...
        if (empty($cast['attributes'])) 
        {
            ...
            if (!empty($cast['variants'])) {
                ...
                $netsuite = !empty($variants[0]['netsuite']) ? $variants[0]['netsuite'] : $netsuite;
            }            

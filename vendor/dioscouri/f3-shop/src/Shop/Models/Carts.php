...
class Carts extends \Dsc\Mongo\Collections\Nodes
{
    ...
    protected function fetchConditions()
    {
        ...
        if ($filter_keyword&&is_string($filter_keyword))
        {
            ...
            if (preg_match($regex, (string) $filter_keyword))
            {
                ...
                $where[] = array(
                    'user_id' => new \MongoId((string) $filter_keyword)
                );                
            }
            $where[] = array(
                'user_email' => $key
    ...
    public static function fetch()
    {
		...
		//print_r($_SESSION['amritaSinghCart']);
		//print_r($_SESSION['amritaSinghCart']);
		if(empty($cart)){
    ...
    protected function afterSave()
    {
		...
		//ob_start();
		//setcookie("amritaSinghCart", serialize($this));
		$_SESSION['amritaSinghCart'] 	=	$this;
		$cookie_string 	=	serialize($this);
		//echo $_COOKIE['amritaSinghCart'];
		setcookie("amritaSinghCart", $cookie_string , time() + (60 * 10));
		//echo $_SESSION['amritaSinghCart'];
		//print_r($_SESSION['amritaSinghCart']);
		//ob_end_flush();
    ...
    public function removeCoupon( $code )
    {
		...
		foreach($this->items as $cartitem=>$itemvalue)
		{
			$discount 	=	"0";
			$this->items[$cartitem]['discount'] 	=	$discount;
			$this->items[$cartitem]['discount_value'] 	=	'0';
		}

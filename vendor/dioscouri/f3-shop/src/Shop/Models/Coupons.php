...
class Coupons extends \Dsc\Mongo\Collections\Describable 
{
    ...
    public function cartValue( \Shop\Models\Carts $cart ) 
    {
		...
		// Start apply the discount coupons item wise:
		
		/*if(!empty($this->discount_applied)){
			foreach($cart->items as $cartitem=>$itemvalue)
			{
				$discount 		=	"0";
				if ($this->discount_type == 'percentage')
				{
					...
					{
						case "order_total":
							$subTotal	=	$cart->calcItemSubtotal( $itemvalue );
				...
				$cart->items[$cartitem]['discount'] 	=	$discount;
			}
			
		}*/
			
		// End apply the discount coupons item wise:
    	...
    	switch ($this->discount_applied) 
    	{
    	    case "order_total":
    	        ...
    	        elseif ($this->discount_type == 'percentage')
    	        {
					...
					foreach($cart->items as $cartitem=>$itemvalue)
					{
						...
						$discount 	= ($this->discount_value/100) * $subTotal;
						$cart->items[$cartitem]['discount'] 	=	$discount;
						$cart->items[$cartitem]['discount_value'] 	=	$this->discount_value;
						$cart->items[$cartitem]['discount_type'] 	=	"percentage";
					}
    	        }
		    ...
		    case "product_total":
		        ...
		        if (empty($target_products))
		        {
		            ...
		            foreach ($cart->items as $cartitemKey => $cartitem)
		            {
		                if (!in_array((string) $cartitem['product_id'], $excluded_products))
		                {
		                    $total = $total + $cart->calcItemSubtotal( $cartitem );
							$discount 	=	"0";
							$subTotal	=	$cart->calcItemSubtotal( $cartitem );
							$discount 	= ($this->discount_value/100) * $subTotal;
							$cart->items[$cartitemKey]['discount'] 	=	$discount;
							$cart->items[$cartitemKey]['discount_value'] 	=	$this->discount_value;
							$cart->items[$cartitemKey]['discount_type'] 	=	"percentage";
		                }	
		            }
		        }
                    ...
                    foreach ($cart->items as $cartitem)
                    {
                        if (!in_array((string) $cartitem['product_id'], $excluded_products))
                        {
                            foreach ($cart->items as $cartitemKey => $cartitem)
                            {
								...
								$discount 	=	"0";
								$subTotal	=	$cart->calcItemSubtotal( $cartitem );
								$discount 	= 	($cartitem['price'] - $this->discount_value) * $cartitem['quantity'];
								$cart->items[$cartitemKey]['discount'] 	=	$discount;
								$cart->items[$cartitemKey]['discount_value'] 	=	$this->discount_value;
								$cart->items[$cartitemKey]['discount_type'] 	=	"flat-rate";
                            }
                        }
                    }
                }

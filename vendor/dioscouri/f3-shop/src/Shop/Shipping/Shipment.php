<?php
/**
 * The shipment class creates a shipment object.
 * Each shipment object will have one or more package objects.
 *
 * At this starting point some code was taken from https://github.com/alexfraundorf-com/ship, Credit where credit is due.
 */
...
class Shipment
{
     ...
     * @since 12/02/2012
     * @param
     *            object \Awsp\Ship\Package $package
     */
     ...
     * @since 12/08/2012
     * @param string $field
     *            the field of the desired property within the shipment_data array
    ...
    /**
     * Goes through each element of $this->shipment_data and applys some basic filtering to it.
     * The elements of
    ...
    protected function isShipmentValid()
    {
        ...
        {
            array_push($required_fields, 'shipping_from_name', 'shipping_from_address1', 'shipping_from_city', 'shipping_from_state', 'shipping_from_postal_code', 'shipping_from_country_code');
        }
        ...
        {
            throw new \UnexpectedValueException('Shipment object is not valid.  Required properties (' . $invalid_properties . ') are not set.');
        }
        else
        {
            return true;

<?php
/**
 * Shipping vendor class for USPS.
 */
namespace Shop\Shipping;

// TODO EVERYTHING NOT EVEN REMOTELY DONE
class Usps implements \Shop\Shipping\ShipperInterface
{
     ...
     * 
     * @param
     *            object \Awsp\Ship\Shipment $Shipment the Shipment object which also contains Package object(s)
     * @param array $config
    ...
    public function getRate()
    {
        ...
        {
            $this->request['Shipment']['ShipFrom']['Address']['PostalCode'] = $this->Shipment->get('shipping_from_postal_code');
            $this->request['Shipment']['ShipFrom']['Address']['CountryCode'] = $this->Shipment->get('shipping_from_country_code');
        }
    ...
    public function createLabel(array $params = array())
    {
        ...
        {
            $this->request['Shipment']['Shipper']['Address']['AddressLine'][] = $this->config['ups']['shipper_address2'];
        }
        if ($this->config['ups']['shipper_address3'] != null)
        {
            $this->request['Shipment']['Shipper']['Address']['AddressLine'][] = $this->config['ups']['shipper_address3'];
        }
        ...
        if ($this->Shipment->get('ship_from_different_address') == true)
        {
            ...
            // attention
            $this->request['Shipment']['ShipFrom']['AttentionName'] = $this->Shipment->get('shipping_from_attention_name');
            $this->request['Shipment']['Shipper']['AttentionName'] = $this->Shipment->get('shipping_from_attention_name');
            // phone
            ...
            // address 1
            $this->request['Shipment']['ShipFrom']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address1');
            ...
            $this->request['Shipment']['Shipper']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address1');
            // address 2
            if ($this->Shipment->get('shipping_from_address2') != null)
            {
                $this->request['Shipment']['ShipFrom']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address2');
                $this->request['Shipment']['Shipper']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address2');
            }
            // address 3
            if ($this->Shipment->get('shipping_from_address3') != null)
            {
                $this->request['Shipment']['ShipFrom']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address3');
                $this->request['Shipment']['Shipper']['Address']['AddressLine'][] = $this->Shipment->get('shipping_from_address3');
            }
            // city
            ...
            // state/province
            $this->request['Shipment']['ShipFrom']['Address']['StateProvinceCode'] = $this->Shipment->get('shipping_from_state');
            $this->request['Shipment']['Shipper']['Address']['StateProvinceCode'] = $this->Shipment->get('shipping_from_state');
            // postal code
            $this->request['Shipment']['ShipFrom']['Address']['PostalCode'] = $this->Shipment->get('shipping_from_postal_code');
            $this->request['Shipment']['Shipper']['Address']['PostalCode'] = $this->Shipment->get('shipping_from_postal_code');
            // country code
            $this->request['Shipment']['ShipFrom']['Address']['CountryCode'] = $this->Shipment->get('shipping_from_country_code');
            $this->request['Shipment']['Shipper']['Address']['CountryCode'] = $this->Shipment->get('shipping_from_country_code');
        }
        ...
        {
            $this->request['Shipment']['ShipTo']['Address']['AddressLine'][] = $this->Shipment->get('receiver_address2');
        }
        if ($this->Shipment->get('receiver_address3') != null)
        {
            $this->request['Shipment']['ShipTo']['Address']['AddressLine'][] = $this->Shipment->get('receiver_address3');
        }
        ...
        if (($this->config['email_tracking_number_to_receiver'] == true) && ($this->Shipment->get('receiver_email') != null))
        {
            ...
            $this->request['Shipment']['ShipmentServiceOptions']['Notification']['EMail']['EMailAddress'] = $this->Shipment->get('receiver_email');
        }
    ...
    protected function sendRequest(array $params = array())
    {
        try
        {
            ...
            // get SOAP response
            $Response = $client->__soapCall($params['operation'], array(
                $this->request
            ));
    ...
    /**
     * Extracts that status of the response and normalizes it.
     ...
     *
     * @param type $messages
     *            the alert section of the SOAP response object
     ...
     *
     * @param object $rate
     *            is an object containing data for a single rate service
     ...
     *            data about the package(s) from the SOAP response
     * @return array
     * @throws \UnexpectedValueException
     * @version updated 01/16/2013
     * @since 12/08/2012
     ...
     *            is an object containing data for a single package
     * @return array containing the extracted data
     * @version updated 12/09/2012
     * @since 12/08/2012
     */

    ...
    <div class="col-md-10">
            ...
            if (!empty($item->id) && ($rebuilt_variants = $item->rebuildVariants()) ) 
            {
                ...
                foreach ($rebuilt_variants as $variant) 
                {
                    ...
                    $key = $variant['key']; 
					//print_r($flash->old('variants.'.$key.'.netsuite.id'));	
                    ?>
                                        ...
                                        <div class="col-md-5">
                                            ...
                                            <input type="text" name="variants[<?php echo $key; ?>][sku]" class="form-control input-sm" value="<?php echo $flash->old('variants.'.$key.'.sku'); ?>" />
											 <?php 
												if($UserRole=='root'){
												?>
												Netsuite ID <input type="text" name="variants[<?php echo $key; ?>][netsuite][id]" value="<?php if($flash->old('variants.'.$key.'.netsuite.id')){ echo $flash->old('variants.'.$key.'.netsuite.id'); } ?>" class="form-control input-sm" />
												<?php 
												}else{
												?> 
